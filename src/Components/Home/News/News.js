...
const News = () => {
	...
	useEffect(() => {
		...
		gsap.to(image3.current, {
			...
			scrollTrigger: { trigger: image3.current, scrub: 0.98, start: 'top bottom', end: 'bottom top' },
		})

		//Image 4
		gsap.to(image4.current, {
			x: () => window.innerWidth * 0.13,
			scrollTrigger: { trigger: image4.current, scrub: 0.98, start: 'top bottom', end: 'bottom top' },
		})
		...
		gsap.fromTo(
			...
			{ rotation: 6, opacity: 0, y: () => heading0.current.clientHeight * 0.5 },
			{
				...
				duration: 0.7,
				ease: 'power4.easeOut',
				scrollTrigger: { trigger: heading1.current, start: 'center bottom' },
			}
		)
