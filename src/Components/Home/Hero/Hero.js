...
const HomeHero = () => {
		...
		setTimeout(() => {
			...
			topSpans.forEach((span, i) => {
				...
				gsap.fromTo(
					...
					{ rotation: 0, y: 0, opacity: 1, duration: 1, ease: 'power4.easeOut', delay: 0.6 + i / 20 }
				)
			})
			...
			headings.forEach((heading, i) => {
				...
				gsap.fromTo(
					...
					{ rotation: 0, y: 0, opacity: 1, duration: 1, ease: 'power4.easeOut', delay: 0.8 + i / 10 }
				)
			})
		}, 2300)
	}, [])
	...
	return (
		<div
			id='hero-container'
			onMouseMove={() => (window.cursorIcon ? window.cursorIcon.show('Scroll') : null)}
